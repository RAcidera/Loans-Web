...
namespace LoanManagementSystem.Application.Promises.Commands.MarkPromiseKept;

public sealed record MarkPromiseKeptCommand(string PromiseId, string PerformedBy) : IRequest<PromiseToPayDto>;

public sealed class MarkPromiseKeptCommandHandler : IRequestHandler<MarkPromiseKeptCommand, PromiseToPayDto>
{
    ...
    public MarkPromiseKeptCommandHandler(
    ...
    public async Task<PromiseToPayDto> Handle(MarkPromiseKeptCommand request, CancellationToken ct)
    {
        ...
        promise.MarkKept(request.PerformedBy);
        await _unitOfWork.SaveChangesAsync(ct); // also flushes PromiseKeptDomainEvent → promise_audit_log
