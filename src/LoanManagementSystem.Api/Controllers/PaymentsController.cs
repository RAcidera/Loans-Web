...
public class PaymentsController : ControllerBase
{
    ...
    /// <summary>GET /api/payments/page?pageIndex=&amp;pageSize=&amp;loanSearch=&amp;customerSearch=&amp;dateFrom=&amp;dateTo=&amp;sortBy=&amp;sortDir= — server-side paging + filtering for the standalone Payments list page.</summary>
    [HttpGet("page")]
    public async Task<ActionResult<PagedResult<PaymentWithCustomerDto>>> GetPage(
        ...
        Ok(await _mediator.Send(new GetPaymentsPageQuery(pageIndex, pageSize, loanSearch, customerSearch, dateFrom, dateTo, sortBy, sortDir), ct));
    ...
    public async Task<ActionResult<PaymentsTotalsDto>> GetPageTotals(
        ...
        Ok(await _mediator.Send(new GetPaymentsTotalsQuery(loanSearch, customerSearch, dateFrom, dateTo), ct));
    ...
    public async Task<IActionResult> Export(
        ...
        [FromQuery] DateOnly? dateFrom = null, [FromQuery] DateOnly? dateTo = null, CancellationToken ct = default)
    {
        var file = await _mediator.Send(new ExportPaymentsXlsxQuery(loanSearch, customerSearch, dateFrom, dateTo), ct);
